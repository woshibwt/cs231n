...
def mutate_tensor(x, indices, values):
  ...
  for idx, indice in enumerate(indices):
    x[indice[0], indice[1]] = values[idx]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def slice_assignment_practice(x):
  ...
  x[:2,:2] = Tensor([0,1])
  x[:2,2:-1] = Tensor([2])
  x[2:-1,0:3:2] = Tensor([3])
  x[2:-1,1:4:2] = Tensor([4])
  x[2:-1,-3:-1] = Tensor([5])
  # x[2:4,:4] = Tensor([3,4])
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx = torch.arange(x.shape[0]).flip(dims=[0])
  y = x[idx,:]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  z = torch.einsum('bnm,bmp->bnp',x, y)
